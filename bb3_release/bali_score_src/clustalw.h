...
#define MAXORGANISMS 60 /* Organism length */
...
#define NUMRES 27 /* max size of comparison matrix */
...
typedef struct node
{ /* phylogenetic tree structure used in readtree.c */
	struct node *left;
	struct node *right;
	struct node *parent;
	float dist;
...
typedef struct
{ /* phylogenetic tree structure used in trees.c */
	char *description;
	double left_branch;
	double right_branch;
} PTREE;
...
typedef struct
{			/* count of number of residues in partial realignment block */
...
void clustal_out(FILE *out, sint fres, sint len, Boolean seq_numbers, ALN mult_aln, SS_OPT ss_opt);
...
void gde_out(FILE *out, sint fres, sint len, Boolean lowercase, ALN mult_aln, SS_OPT ss_opt);
...
int read_secator_groups(ALN mult_aln, double **tmat, char *filename, GROUP *groups, sint *secgroup, sint *orggroup);
