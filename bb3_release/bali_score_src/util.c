...
void create_parameter_output(OPT opt, ALN mult_aln)
{
        ...
        */
        ...
        if ((parout = open_explicit_file(parname)) == NULL)
                return;

        fprintf(parout, "clustalw \\\n");
        if ((mult_aln.nseqs > 0) && (mult_aln.prf1.nseqs <= 0))
                fprintf(parout, "-infile=%s \\\n", mult_aln.filename);
        if (mult_aln.prf1.nseqs > 0)
                fprintf(parout, "-profile1=%s\\\n", mult_aln.prf1.filename);
        if (mult_aln.prf2.nseqs > 0)
                fprintf(parout, "-profile2=%s\\\n", mult_aln.prf2.filename);
        if (mult_aln.dnaflag == TRUE)
        ...
        if (opt.pw_opt->quick_pairalign)
        {
                ...
                if (opt.quickpw_opt->percent)
                        fprintf(parout, "/score=percent \\\n");
                else
        ...
        if (!opt.mult_opt->gap_opt->use_endgaps)
                fprintf(parout, "-endgaps \\\n");

        if (!mult_aln.dnaflag)
        {
                if (opt.mult_opt->neg_matrix)
                        fprintf(parout, "-negative \\\n");
                if (opt.mult_opt->gap_opt->no_pref_penalties)
                        fprintf(parout, "-nopgap \\\n");
                if (opt.mult_opt->gap_opt->no_hyd_penalties)
                        fprintf(parout, "-nohgap \\\n");
                if (opt.mult_opt->gap_opt->no_var_penalties)
                        fprintf(parout, "/novgap \\\n");
        ...
        if (opt.alnout_opt->output_gcg)
                fprintf(parout, "-output=gcg \\\n");
        else if (opt.alnout_opt->output_gde)
                fprintf(parout, "-output=gde \\\n");
        else if (opt.alnout_opt->output_nbrf)
                fprintf(parout, "-output=pir \\\n");
        else if (opt.alnout_opt->output_phylip)
                fprintf(parout, "-output=phylip \\\n");
        if (opt.alnout_opt->output_order == ALIGNED)
                fprintf(parout, "-outorder=aligned \\\n");
        else
                fprintf(parout, "-outorder=input \\\n");
        if (opt.alnout_opt->output_gde)
                if (opt.alnout_opt->lowercase)
...
void remove_gap_pos(sint prf_no, ALNPTR mult_aln)
{
        ...
        for (i = 0; i < mult_aln->seqs[fseq].len;)
        {
                ngaps = 0;
                for (j = fseq; j < lseq; j++)
                        if (!isalpha(mult_aln->seqs[j].data[i]))
                                ngaps++;
                if (ngaps == lseq - fseq)
                {
                        for (j = fseq; j < lseq; j++)
...
void add_ft_entry(ALNPTR mult_aln, sint seq, sint first, sint last, sint type, sint code, float score, char *ctype, char *iname, sint is, sint ie)
{
        ...
        {
                if (strncmp(name, "PRED_HELIX", 10) == 0)
                        color = VLRED;
                else if (strncmp(name, "PRED_STRAND", 11) == 0)
                        color = VLGREEN;
                else if (strncmp(name, "PROP_HELIX", 10) == 0)
                        color = LRED;
                else if (strncmp(name, "PROP_STRAND", 11) == 11)
                        color = LGREEN;
        ...
        else if (strcmp(ctype, "TRANSMEM") == 0)
        {
                ...
                else if (strncmp(name, "PROP_", 5) == 0)
                        color = GRAY;
                else
                        color = 1;
        }
        ...
        else if (strcmp(ctype, "DOMAIN") == 0)
        {
                if (strncmp(name, "PRED_", 5) == 0)
                        ...
                        color = GRAY;
                else if (strncmp(name, "DNA_BIND", 8) == 0)
                        color = 2;
                else if (strncmp(name, "ZN_FING", 7) == 0)
                        color = 3;
                else
                        color = 1;
        }
        ...
        else if (strcmp(ctype, "SIGNAL") == 0)
        {
                if (strncmp(name, "PRED_", 5) == 0)
                {
                        ...
                        color = GRAY;
                else if (strncmp(name, "NUCLEAR", 7) == 0)
                        color = 1;
                else if (strncmp(name, "CHLOROPLAST", 11) == 0)
                        color = 2;
                else if (strncmp(name, "MITOCHONDRION", 13) == 0)
                        color = 3;
                else if (strncmp(name, "MICROBODY", 9) == 0)
                        color = 4;
                else
                        color = 5;
        }
        ...
        else if (strcmp(ctype, "MOD_RES") == 0)
        {
                ...
                if (strncmp(name, "PRED_", 5) == 0)
                        ...
                        color = GRAY;
                else if (strncmp(name, "ACETYLATION", 11) == 0)
                        color = 1;
                else if (strncmp(name, "ALKYLATION", 10) == 0)
                        color = 2;
                else if (strncmp(name, "HYDROXYLATION", 13) == 0)
                        color = 3;
                else if (strncmp(name, "METHYLATION", 11) == 0)
                        color = 4;
                else if (strncmp(name, "PHOSPHORYLATION", 15) == 0)
                        color = 5;
                else if (strncmp(name, "SULFATION", 9) == 0)
                        color = 6;
                else if (strncmp(name, "PYRROLIDONE", 11) == 0)
                        color = 7;
                else if (strncmp(name, "MYRISTATE", 9) == 0)
                        color = 8;
                else if (strncmp(name, "PALMITATE", 9) == 0)
                        color = 9;
                else if (strncmp(name, "FARNESYL", 8) == 0)
                        color = 10;
                else if (strncmp(name, "GERANYL", 7) == 0)
                        color = 11;
                else if (strncmp(name, "GPI-ANCHOR", 3) == 0)
                        color = 12;
                else if (strncmp(name, "N-ACYL", 5) == 0)
                        color = 13;
        }
        else if (strcmp(ctype, "SITE") == 0)
        {
                ...
                if (strncmp(name, "PRED_", 5) == 0)
                        ...
                        color = GRAY;
                else if (strncmp(name, "SITE", 4) == 0)
                        color = 0;
                else if (strncmp(name, "ACT_SITE", 8) == 0)
                        color = 1;
                else if (strncmp(name, "CARBOHYD", 8) == 0)
                        color = 2;
                else if (strncmp(name, "DISULFID", 8) == 0)
                        color = 3;
                else if (strncmp(name, "BINDING", 7) == 0)
                        color = 4;
                else if (strncmp(name, "NP_BIND", 7) == 0)
                        color = 5;
                else if (strncmp(name, "CA_BIND", 7) == 0)
                        color = 6;
                else if (strncmp(name, "TRANSIT", 7) == 0)
                        color = 7;
                else if (strncmp(name, "LIPID", 5) == 0)
                        color = 8;
                else if (strncmp(name, "METAL", 5) == 0)
                        color = 9;
        }
