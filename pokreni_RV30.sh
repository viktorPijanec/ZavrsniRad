...
for tfa_file in bb3_release/RV30/*.tfa; do
    ...
    build/msa "$tfa_file"
    bb3_release/bali_score_src/bali_score "bb3_release/RV30/${base_name}.msf" izgraden_msa.txt > pom1.txt
    
    # Extract scores
    sp_score=$(grep -oP 'SP score=\s*\K\d+\.\d+' pom1.txt)
    echo "normalni sp score $sp_score"
    tc_score=$(grep -oP 'TC score=\s*\K\d+\.\d+' pom1.txt)
    ...
    bb3_release/bali_score_src/bali_score bb3_release/RV30/${base_name}.msf bb3_release/RV30/${base_name}.aln > pom1.txt
    sp_score=$(grep -oP 'SP score=\s*\K\d+\.\d+' pom1.txt)
...
echo "RV30: SP=$sp_score_RV11/$brojac_RV11, TC=$tc_score_RV11/$brojac_RV11"
echo "RV30_nj: SP=$sp_score_nj_RV11/$brojac_RV11, TC=$tc_score_nj_RV11/$brojac_RV11"
echo "RV30_clustal: SP=$sp_score_clustal_RV11/$brojac_RV11, TC=$tc_score_clustal_RV11/$brojac_RV11"