...
for tfa_file in bb3_release/RV40/*.tfa; do
    ...
    build/msa -o pomocni40.txt "$tfa_file"
    bb3_release/bali_score_src/bali_score "bb3_release/RV40/${base_name}.msf" pomocni40.txt > pom2.txt
    
    # Extract scores
    sp_score=$(grep -oP 'SP score=\s*\K\d+\.\d+' pom2.txt)
    echo "normalni sp score $sp_score"
    tc_score=$(grep -oP 'TC score=\s*\K\d+\.\d+' pom2.txt)
    ...
    bb3_release/bali_score_src/bali_score bb3_release/RV40/${base_name}.msf bb3_release/RV40/${base_name}.aln > pom2.txt
    sp_score=$(grep -oP 'SP score=\s*\K\d+\.\d+' pom2.txt)
...
echo "RV40: SP=$sp_score_RV11/$brojac_RV11, TC=$tc_score_RV11/$brojac_RV11"
echo "RV40_nj: SP=$sp_score_nj_RV11/$brojac_RV11, TC=$tc_score_nj_RV11/$brojac_RV11"
echo "RV40_clustal: SP=$sp_score_clustal_RV11/$brojac_RV11, TC=$tc_score_clustal_RV11/$brojac_RV11"